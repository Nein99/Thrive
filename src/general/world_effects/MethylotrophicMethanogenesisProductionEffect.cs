...
public class MethylotrophicMethanogenesisProductionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public MethylotrophicMethanogenesisProductionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "methylotrophic_methanogenesis")
                        _ = 1;
                    ...
                    foreach (var input in process.Process.Inputs)
                    {
                        ...
                        else if (input.Key.ID is Compound.Oxygen)
                        {
                    ...
                    foreach (var output in process.Process.Outputs)
                    {
                        ...
                        else if (output.Key.ID is Compound.Oxygen)
                        {
    ...
    private bool IsProcessRelevant(BioProcess process)
    {
        ...
        {
            if (input.Key.ID is Compound.Methane or Compound.Carbondioxide or Compound.Oxygen)
                return true;
        }

        foreach (var output in process.Outputs)
        {
            if (output.Key.ID is Compound.Methane or Compound.Carbondioxide or Compound.Oxygen)
                return true;
        }
