...
public class AcetogenesisProductionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public AcetogenesisProductionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "acetogenesis")
                        _ = 1;
                    ...
                    {
                        if (input.Key.ID is Compound.Hydrogen)
                        {
                    ...
                    {
                        if (output.Key.ID is Compound.Hydrogen)
                        {
    ...
    private bool IsProcessRelevant(BioProcess process)
    {
        ...
        {
            if (input.Key.ID is Compound.Hydrogen or Compound.Carbondioxide)
                return true;
        }

        foreach (var output in process.Outputs)
        {
            if (output.Key.ID is Compound.Hydrogen or Compound.Carbondioxide)
                return true;
        }
