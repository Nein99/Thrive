...
public class AcetoclasticMethanogenesisProductionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public AcetoclasticMethanogenesisProductionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "acetoclastic_methanogenesis")
                        _ = 1;
    ...
    private bool IsProcessRelevant(BioProcess process)
    {
        ...
        {
            if (input.Key.ID is Compound.Methane or Compound.Carbondioxide)
                return true;
        }

        foreach (var output in process.Outputs)
        {
            if (output.Key.ID is Compound.Methane or Compound.Carbondioxide)
                return true;
        }
