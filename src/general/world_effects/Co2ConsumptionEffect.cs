...
public class Co2ConsumptionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public Co2ConsumptionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "iron_chemolithoautotrophy")
                        _ = 1;

                    if (process.Process.InternalName == "manganese_chemolithoautotrophy")
                        _ = 1;

                    if (process.Process.InternalName == "chemo_synthesis")
                        _ = 1;
                    ...
                    {
                        if (input.Key.ID is Compound.Carbondioxide)
                        {
                    ...
                    {
                        if (output.Key.ID is Compound.Carbondioxide)
                        {
