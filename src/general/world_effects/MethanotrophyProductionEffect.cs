...
public class MethanotrophyProductionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public MethanotrophyProductionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "methanotrophy")
                        _ = 1;
