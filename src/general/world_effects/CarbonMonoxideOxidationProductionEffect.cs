...
public class CarbonMonoxideOxidationProductionEffect : IWorldEffect
{
    [JsonProperty]
    private GameWorld targetWorld;

    public CarbonMonoxideOxidationProductionEffect(GameWorld targetWorld)
    {
    ...
    private void ApplyCompoundsAddition()
    {
        ...
        foreach (var patchKeyValue in targetWorld.Map.Patches)
        {
            ...
            foreach (var species in patch.SpeciesInPatch)
            {
                ...
                {
                    if (process.Process.InternalName == "carbon_monoxide_oxidation")
                        _ = 1;
    ...
    private bool IsProcessRelevant(BioProcess process)
    {
        ...
        {
            if (input.Key.ID is Compound.Carbonmonoxide or Compound.Carbondioxide)
                return true;
        }

        foreach (var output in process.Outputs)
        {
            if (output.Key.ID is Compound.Carbonmonoxide or Compound.Carbondioxide)
                return true;
        }
